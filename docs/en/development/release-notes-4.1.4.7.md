...
| Stage dependency | Minimum version is `0.3.6` in the existing `<0.4.0` compatibility line | Updating Stage alone fixes the existing script; updating Agently fixes dependency resolution for later installs |
